		...
		while (vertex_id < vertex_offset + num_vertexes)
		{
			...
			for (int x = static_cast<int>(bounding_box_begin.x); x <= static_cast<int>(bounding_box_end.x); ++x)
			{
				for (int y = static_cast<int>(bounding_box_begin.y); y <= static_cast<int>(bounding_box_end.y); ++y)
				{
					...
					float edge0 = edge_function(float2{vertices[0].x, vertices[0].y}, float2{vertices[1].x, vertices[1].y}, point);
					float edge1 = edge_function(float2{vertices[1].x, vertices[1].y}, float2{vertices[2].x, vertices[2].y}, point);
					float edge2 = edge_function(float2{vertices[2].x, vertices[2].y}, float2{vertices[0].x, vertices[0].y}, point);
